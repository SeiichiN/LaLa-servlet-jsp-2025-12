...
public class Player {
	...
	public void printInfo() {
		gm.out.println("HP:" + this.hp + "/" + this.MAX_HP);
		printItems();
	}
	...
	public void attack(Monster m) {
		...
		m.hp -= damage;
		gm.out.println(m.name + "は" + damage + "のダメージを受けた!");
	}
	...
	public void take() {
		...
		if (item == null) return;
		gm.out.println(this.name + "は" + item.name + "を手に入れた!");
		this.items.add(item);
		gm.map[py][px] = '.';
	}
	...
	public void printItems() {
		...
		for (int i = 0; i < items.size(); i++) {
			gm.out.print(i + 1 + ":" + items.get(i).name + " ");
		}
		gm.out.println();
	}
	...
	public void usePotion(Item item) {
		...
		if (item instanceof Potion) {
			...
			gm.out.println("HPが" + (this.hp - nowHP) + "回復した!");
			items.remove(item);
		}
	}
	...
	public void use() {
		if (this.items.size() == 0) {
			gm.out.println("持ち物がありません");
			return;
		}
		Item item = selectItem();
		gm.out.println(this.name + "は" + item.name + "を使った!");
		switch (item.suffix) {
